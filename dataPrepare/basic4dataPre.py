...
# @Last modified time: 2020-04-08T10:08:35+02:00
...
def patch2labelInx_lt(y, startPixel, endPixel, ltValue):
	...
	b=np.min(y[:, startPixel:endPixel, startPixel:endPixel, 0], axis=1);
	percentageB=np.min(b, axis=1).reshape(y.shape[0],1)

	idx_densB=np.where(percentageB<ltValue)
	idx = idx_densB[0]

	return idx
def patch2labelInx_gt(y, startPixel, endPixel, Value):

	b=np.max(y[:, startPixel:endPixel, startPixel:endPixel, 0], axis=1);
	percentageB=np.max(b, axis=1).reshape(y.shape[0],1)

	idx_densB=np.where(percentageB>Value)
	idx = idx_densB[0]
...
def savePatch_fold(patchX, patchLCZ, patchHSE, saveFolder):
    num=patchX.shape[0]
    # numV=np.int64( np.round(num*0.05) )
    indexRandom=np.arange(num, dtype=np.int64)#.reshape(y_tra.shape[0],1)
    np.random.shuffle(indexRandom)

    save2MultipleFile_(patchX[indexRandom, :, :, :], patchLCZ[indexRandom, :, :, :], patchHSE[indexRandom, :, :, :], saveFolder)

    # return num-numV, numV

def save2MultipleFile_(patchX, patchLCZ, patchHSE, saveFolder, numFile=100):
    num=patchX.shape[0]
    numEach=np.int64( np.round(num*1/numFile) )
    ...
    for f in np.arange(numFile):
        ...
        if f==numFile-1:
            ...
            hf.create_dataset('y_0', data= patchHSE[idxStart:, :, :, :])
            hf.create_dataset('y_1', data= patchLCZ[idxStart:, :, :, :])
        else:
            hf.create_dataset('x', data= patchX[idxStart:(idxStart+numEach), :, :, :])
            hf.create_dataset('y_0', data= patchHSE[idxStart:(idxStart+numEach), :, :, :])
            hf.create_dataset('y_1', data= patchLCZ[idxStart:(idxStart+numEach), :, :, :])

        hf.close()
        # data= patchLCZ[idxStart:(idxStart+numEach), :, :, :]
        # print(data.shape)
        # print(f, idxStart, idxStart+numEach, num, numEach, patchX.shape[0], patchHSE.shape[0], patchLCZ.shape[0])
        idxStart=idxStart+numEach
