...
for nn in args.methods4test:
	...
	fileSave=  args.saveFolder + str(learnRate) + '_' + str(nn) + '_' + str(batch_size)
	...
	checkpoint_0 = ModelCheckpoint(fileSave+"weights.best_hse.hdf5", monitor='val_hse_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=1)
	checkpoint_1 = ModelCheckpoint(fileSave+"weights.best_lcz.hdf5", monitor='val_lcz_loss', verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=1)
	#-{epoch:02d}-{val_loss:.2f}
	# checkpoint_ = ModelCheckpoint(fileSave+'model{epoch:03d}.h5', period=5)

	tbCallBack = TensorBoard(log_dir=args.saveFolder+'logs' + '_' + str(nn) + '_' + str(batch_size) + '_' + timeCreated,  # log 目录
	...
	###################################################################
	model, outNumber, dataGenerFlow = modelS.modelSelection(nn, patch_shape, learnRate)
	...
	model.fit_generator(generator = tra_generator,
					...
					callbacks = [checkpoint, checkpoint_0, checkpoint_1, tbCallBack, early_stopping, lr_sched], max_queue_size = 100, verbose=1)#lr_sched
	end =time.time()
